...
【設計思想】
- 100点満点からスタート（4TF Perfect Order = 完璧な状態）
- row_orderを使用してPerfect Order判定
- 各種要因で減点していく方式
- 矛盾（dauten vs PO方向）にペナルティ
"""
...
# ============================================================
DEDUCTION_CONFIG = {
    # =============== A. Perfect Order評価（主軸） ===============
    'perfect_order': {
        '4tf_perfect': 0,        # 4TF完全整列: 減点なし
        '3tf_aligned': -20,      # 3TF整列: -20点
        '2tf_aligned': -45,      # 2TF整列: -45点
        '1tf_only': -60,         # 1TFのみ: -60点
        'no_alignment': -70,     # 整列なし: -70点
        'no_row_order': -80,     # row_orderデータなし: -80点
    },
    
    # =============== B. 角度の弱さ ===============
    'angle_weakness': {
        'strong': (35, 0),         # |angle| >= 35°: 0点
        'moderate': (25, -3),      # 25° <= |angle| < 35°: -3点
        'weak': (15, -8),          # 15° <= |angle| < 25°: -8点
        'very_weak': (5, -15),     # 5° <= |angle| < 15°: -15点
        'flat': (0, -25),          # |angle| < 5°: -25点
    },
    
    # =============== C. 厚みの弱さ ===============
    'thickness_weakness': {
        'thick': (50, 0),          # thickness >= 50: 0点
        'moderate': (30, -3),      # 30 <= thickness < 50: -3点
        'thin': (15, -8),          # 15 <= thickness < 30: -8点
        'very_thin': (5, -12),     # 5 <= thickness < 15: -12点
        'ultra_thin': (0, -20),    # thickness < 5: -20点
    },
    
    # =============== D. 矛盾ペナルティ ===============
    'contradiction': {
        'dauten_mismatch': -15,     # dauten方向不一致: -15点
        'angle_mismatch': -15,      # angle符号不一致: -15点
        'gc_dc_mismatch': -12,      # GC/DC矛盾: -12点
        'multiple': -30,            # 複数矛盾: -30点（個別合計の代わり）
    },
    
    # =============== E. データ欠損 ===============
    'data_absence': {
        'gc_dc_none': -8,           # GC=None, DC=None: -8点
        'dauten_none': -10,         # dauten=None: -10点
        'row_order_empty': -15,     # row_order='': -15点
        'clouds_empty': -80,        # clouds=[]: -80点
    },
    
    # =============== F. 上位足の影響 ===============
    'upper_tf_influence': {
        '1level_opposite': -10,     # 1段階上が逆: -10点
        '2level_opposite': -5,      # 2段階上が逆: -5点
        'no_upper_data': -3,        # 上位データなし: -3点
    },
    
    # =============== G. 距離の問題 ===============
    'distance_issue': {
        'very_close': (15, 0),      # <15 Pips: 0点
        'close': (30, -2),          # 15-30 Pips: -2点
        'normal': (50, -5),         # 30-50 Pips: -5点
        'far': (70, -10),           # 50-70 Pips: -10点
        'very_far': (999, -15),     # ≥70 Pips: -15点
    },
...
def calculate_trend_strength_v2(tf, state_data, all_states=None):
    ...
    Args:
        tf: 対象時間足（'5', '15', '60', '240'）
        state_data: 当該時間足の状態データ
        all_states: 全タイムフレームの状態データ（上位足判定用）
    ...
    try:
        ...
        distance_from_prev = abs(cloud.get('distance_from_prev', 0))  # 絶対値化
        dauten = cloud.get('dauten', None)
        gc = cloud.get('gc', None)
        dc = cloud.get('dc', None)
        row_order = state_data.get('row_order', '')
        
        # ============================================================
        # ステップ 2: Perfect Order判定（主軸）
        # ============================================================
        po_direction, po_level, po_deduction = _evaluate_perfect_order(
            row_order, tf, all_states
        )
        ...
        score = 100
        deduction_breakdown = {
            'perfect_order': po_deduction
        }
        
        # ============================================================
        # ステップ 4: 角度の弱さで減点
        # ============================================================
        angle_deduction = _evaluate_angle_weakness(angle)
        score += angle_deduction
        deduction_breakdown['angle_weakness'] = angle_deduction
        
        # ============================================================
        # ステップ 5: 厚みの弱さで減点
        # ============================================================
        thickness_deduction = _evaluate_thickness_weakness(thickness)
        score += thickness_deduction
        deduction_breakdown['thickness_weakness'] = thickness_deduction
        
        # ============================================================
        # ステップ 6: 矛盾ペナルティ
        # ============================================================
        contradiction_deduction, contradiction_details = _evaluate_contradictions(
            po_direction, dauten, angle, gc
        )
        score += contradiction_deduction
        deduction_breakdown['contradictions'] = contradiction_deduction
        deduction_breakdown['contradiction_details'] = contradiction_details
        
        # ============================================================
        # ステップ 7: データ欠損で減点
        # ============================================================
        data_absence_deduction = _evaluate_data_absence(
            dauten, row_order, clouds
        )
        score += data_absence_deduction
        deduction_breakdown['data_absence'] = data_absence_deduction
        
        # ============================================================
        # ステップ 8: 上位足の影響
        # ============================================================
        upper_deduction = _evaluate_upper_tf_influence(
            tf, po_direction, all_states
        )
        score += upper_deduction
        deduction_breakdown['upper_tf_influence'] = upper_deduction
        
        # ============================================================
        # ステップ 9: 距離の問題
        # ============================================================
        distance_deduction = _evaluate_distance_issue(distance_from_prev)
        score += distance_deduction
        deduction_breakdown['distance_issue'] = distance_deduction
        
        # Perfect Orderの減点を最後に適用
        score += po_deduction
